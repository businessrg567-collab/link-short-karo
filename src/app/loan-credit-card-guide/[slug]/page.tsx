...
import { loanGuideArticles, getLoanGuideArticleBySlug } from '@/data/loanGuideData';
...
export default async function LoanArticlePage({ params }: ArticlePageProps) {
    const { slug } = await params;
    const articleData = getLoanGuideArticleBySlug(slug);
    ...
    const related = loanGuideArticles.filter(a => a.slug !== articleData.slug).slice(0, 4);
            ...
            <div className="container">
                    ...
                    <aside className={styles.sidebar}>
                        <div style={{ position: 'sticky', top: '100px' }}>
                            ...
                            <div className={styles.sidebarCard} style={{ marginTop: '2rem' }}>
                                <h3>Finance Guides</h3>
                                <ul>
                                    {loanGuideArticles.filter(a => a.slug !== articleData.slug).slice(0, 5).map(a => (
                                        <li key={a.slug}>
                                            <a href={`/loan-credit-card-guide/${a.slug}`}>{a.title}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                {related.length > 0 && (
                    <section style={{ borderTop: '2px solid hsl(var(--border))', paddingTop: '3rem', marginTop: '3rem' }}>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.5rem' }}>More Finance & Loan Guides</h2>
                        <p style={{ color: 'hsl(var(--muted-foreground))', marginBottom: '2rem' }}>Smart borrowing tips and financial guides for every Indian</p>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1.25rem' }}>
                            {related.map((a) => (
                                <a key={a.slug} href={`/loan-credit-card-guide/${a.slug}`} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', overflow: 'hidden' }}>
                                    <div style={{ height: '150px', overflow: 'hidden' }}>
                                        <SafeImage src={a.image} alt={a.title} className={styles.relatedImage} />
                                    </div>
                                    <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                        {a.tag && <span style={{ background: '#1e3a5f', color: '#60a5fa', padding: '0.15rem 0.6rem', borderRadius: '4px', fontSize: '0.72rem', fontWeight: 700, marginBottom: '0.5rem', display: 'inline-block' }}>{a.tag}</span>}
                                        <h3 style={{ fontSize: '0.92rem', fontWeight: 700, color: 'hsl(var(--foreground))', marginBottom: '0.5rem' }}>{a.title}</h3>
                                        <span style={{ color: '#2563eb', fontWeight: 700, fontSize: '0.82rem', marginTop: 'auto', paddingTop: '0.5rem', display: 'block' }}>Read Guide →</span>
                                    </div>
                                </a>
                            ))}
                        </div>
